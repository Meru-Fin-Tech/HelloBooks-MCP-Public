...
import { createDirectoryLoader, queryAccountants, accountantSchema } from '../src/accountants.js';
...
test('all full profiles remain reachable across pagination, with availability optional', async () => {
  ...
  const load = createDirectoryLoader({ fetcher: (async () => Response.json({ firms: source })) as typeof fetch });
...
test('a successful empty result replaces previous data; outages retain marked stale data', async () => {
  let time = 0, outcome: unknown = { firms: [firm(1)] }, calls = 0;
...
test('profile and list rendering escape HTML, reject executable URLs and preserve full details', async () => {
  ...
  const load = createDirectoryLoader({ fetcher: (async () => Response.json({ firms: [f] })) as typeof fetch });
