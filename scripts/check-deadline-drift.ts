...
// Bounded `[^>]{0,4096}` instead of `[^>]+` prevents pathological backtracking
// on an unclosed `<` followed by megabytes of non-`>` input (SonarQube S5852).
// A 4 KiB tag length is well above any real HTML attribute payload.
export function stripHtml(html: string): string {
  return html
    .replaceAll(/<script[\s\S]{0,1048576}?<\/script>/gi, ' ')
    .replaceAll(/<style[\s\S]{0,1048576}?<\/style>/gi, ' ')
    .replaceAll(/<[^>]{0,4096}>/g, ' ')
    .replaceAll(/&nbsp;/gi, ' ')
    .replaceAll(/&amp;/gi, '&')
    .replaceAll(/&lt;/gi, '<')
...
export function pageContainsDueDayNearForm(
  ...
  const formNorm = form.toLowerCase().replaceAll(/\s+/g, ' ').trim();
  const dayPatterns = [
    new RegExp(String.raw`\b${dueDay}\b`),
    new RegExp(String.raw`\b${dueDay}${ordinal(dueDay).slice(String(dueDay).length)}\b`),
  ];
  let cursor = 0;
  let found = false;
...
function formatReport(
  ...
  const lines: string[] = [];
  lines.push(
    `# Compliance deadline drift report — ${runDate}`,
    '',
  ...
  if (fetchFailures.length > 0) {
    ...
    for (const f of fetchFailures) {
      lines.push(
        `- **${f.id}** (${f.country} — ${f.form}) — ${f.source}`,
        `  - ${f.detail}`,
      );
    }
    lines.push('');
  ...
  if (findings.length === 0) {
    lines.push(
      '## Result',
      '',
      'All scanned entries still match their authority sources. No action required.',
      '',
    );
  }
...
async function main(): Promise<void> {
  ...
  for (const d of COMPLIANCE_DEADLINES) {
    ...
    if (!page || !page.ok) {
      const errSuffix =
        page?.status === 0 ? ` (${page.text.slice(0, 200)})` : '';
      findings.push({
        id: d.id,
        country: d.country,
        form: d.form,
        ...
        detail: `fetch returned status ${page?.status ?? 'unknown'}${errSuffix}`,
      });
      continue;
    }
