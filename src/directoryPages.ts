...
export function renderPage(title: string, body: string): string {
  ...
  *{box-sizing:border-box}body{font:16px/1.6 system-ui,sans-serif;margin:0;background:#f7f9fc;color:#15213b}
  ...
  .brand{color:#15213b;font-weight:750;text-decoration:none}h1{font-size:clamp(1.8rem,4vw,2.6rem);line-height:1.2}h2{line-height:1.35}p{max-width:80ch}.muted{color:#526179}.eyebrow{font-size:13px;text-transform:uppercase;letter-spacing:.1em;font-weight:700;color:#385b90}
...
export function renderAccountants(result: ReturnType<typeof queryAccountants>, params: URLSearchParams): string {
  ...
  <label>Availability<select name="acceptingClients"><option value="">All published firms</option value="true" ${params.get('acceptingClients') === 'true' ? 'selected' : ''}>Accepting clients</option><option value="false" ${params.get('acceptingClients') === 'false' ? 'selected' : ''}>Not accepting clients</option></select></label><button type="submit">Search directory</button><a href="/accountants">Clear filters</a></div></form>
