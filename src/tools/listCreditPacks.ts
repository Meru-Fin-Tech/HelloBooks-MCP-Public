...
export function listCreditPacks(args: ListCreditPacksArgs) {
  ...
  return {
    ...
    note: 'Credit packs are one-time pay-as-you-go top-ups of AI credits, purchasable on top of any plan including Free. Prices are list prices in local currency. `dataSource` is "live-feed" when fetched from hellobooks.ai/api/feed/pricing.json, or "static-fallback" when served from the baked catalog.',
  };
}
